...
const DECIMAL = web3.utils.toBN(web3.utils.toWei('1'));
const AMOUNT_DAI_WEI = web3.utils.toBN(web3.utils.toWei('100000'));
const DIRECTION = {
  KYBER_TO_UNISWAP: 0,
  UNISWAP_TO_KYBER: 1
};
...
const init = async () => {
  ...
  const updateEthPrice = async () => {
    const results = await kyber.methods.getExpectedRate(
        ...
        1).call();
    ethPrice = web3.utils.toBN('1').mul(web3.utils.toBN(results.expectedRate)).div(DECIMAL);
  }
  ...
  web3.eth.subscribe('newBlockHeaders')
    ...
    // Calculating amount of eth we will get from Kyber and Uniswap for amount of Dai we have
      ...
      const ethFromKyber = AMOUNT_DAI_WEI.mul(web3.utils.toBN(amountsEth[0].expectedRate)).div(DECIMAL);
      const ethFromUniswap = web3.utils.toBN(amountsEth[1][0].raw.toString());

      //Calculating amount of Dai we will receive after swapping ether on kyber and uniswap
      ...
      const daiFromKyber = ethFromUniswap.mul(web3.utils.toBN(amountsDai[0].expectedRate)).div(DECIMAL);
      ...
      if(daiFromUniswap.gt(AMOUNT_DAI_WEI)) {
        ...
        const txCost = web3.utils.toBN(gasAmount).mul(web3.utils.toBN(gasPrice)).mul(ethPrice);
